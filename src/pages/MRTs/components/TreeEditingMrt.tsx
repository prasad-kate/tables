...
const TreeEditingMrt = () => {
  ...
  const columns = useMemo<MRT_ColumnDef<AnotherUser>[]>(
    () => [
      {
        accessorKey: "id",
        header: "Id",
  ...
  //CREATE action
  const handleCreateUser: MRT_TableOptions<AnotherUser>["onCreatingRowSave"] =
  ...
  //UPDATE action
  const handleSaveUser: MRT_TableOptions<AnotherUser>["onEditingRowSave"] =
    async ({ values, table }) => {
      const newValidationErrors = validateUser(values);
      if (Object.values(newValidationErrors).some((error) => error)) {
        setValidationErrors(newValidationErrors);
  ...
  //DELETE action
  const openDeleteConfirmModal = (row: MRT_Row<AnotherUser>) => {
...
function useCreateUser() {
  ...
  return useMutation({
    mutationFn: async (user: AnotherUser) => {
    ...
    onMutate: (newUserInfo: AnotherUser) => {
      queryClient.setQueryData(["users"], (_prevUsers: AnotherUser[]) => {
        const prevUsers: AnotherUser[] = JSON.parse(JSON.stringify(_prevUsers));
        newUserInfo.subRows = [];
...
function useGetUsers() {
  return useQuery<AnotherUser[]>({
    queryKey: ["users"],
    queryFn: async () => {
      //send api request here
      await new Promise((resolve) => setTimeout(resolve, 1000)); //fake api call
      return Promise.resolve(anotherFakeData);
    },
...
function useDeleteUser() {
  ...
  return useMutation({
    ...
    onMutate: (userId: string) => {
      queryClient.setQueryData(["users"], (prevUsers: any) => {
        const newUsers: AnotherUser[] = JSON.parse(JSON.stringify(prevUsers));
        //remove user
...
const TreeEditingTypeTable = () => (
  //Put this with your other react-query providers near root of your app
  <QueryClientProvider client={queryClient}>
    <div className="table-container">
      <Title title="Tree Editing" />
      <TreeEditingMrt />
    </div>
  </QueryClientProvider>
);

export default TreeEditingTypeTable;

const validateRequired = (value: string) => !!value.length;

function validateUser(user: AnotherUser) {
  return {
